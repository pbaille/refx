...
(defn init [{:as frame :keys [registry app-db dispatcher]}]

  (let [{:keys [dispatch dispatch-sync]} dispatcher
        do-fx
        ...
        (->interceptor
         ...
         :after (fn do-fx-after
                  ...
                  (let [effects            (:effects context)
                        ...
                        db-effect          (registry/lookup registry kind :db)]
        ;; :db effect is guaranteed to be handled before all other effects.
                    ...
                    (doseq [[fx-id effect-value] effects-without-db]
                      (if-let [effect-fn (registry/lookup registry kind fx-id false)]
...
;; :deregister-event-handler
    ...
    (register
     ...
     (fn [value]
       (if (sequential? value)
         (doseq [event-id value] (registry/remove! registry events/kind event-id))
